...
def preprocess_uploaded_3d_array(array):
    ...
    # Remove batch dimension: [4, 16, 96, 96]
...
def render_workflow_buttons(workflow):
    ...
    with c1:
        st.button("← Home", on_click=go_to_page, args=("Home",))
...
if page == "Home":
    ...
    with col1:
        ...
        st.caption("Upload a single MRI image, run segmentation, view Grad-CAM, and check 2D training results.")

    with col2:
        st.button("3D MRI Workflow", on_click=go_to_page, args=("3D Info",))
        st.caption("Run volume-based MRI segmentation, inspect slices, view 3D output, and check 3D training results.")
...
# =========================
    ...
    if "result_3d" in st.session_state:
        ...
        st.session_state["export_3d"] = {
            "input_img": image_to_png_bytes(input_img),
            "pred_mask_img": image_to_png_bytes(pred_mask_img),
            "overlay_img": image_to_png_bytes(overlay_img),
            "true_mask_img": image_to_png_bytes(true_mask_img) if true_mask_img is not None else None,
            "prob_img": image_to_png_bytes(prob_img),
        ...
        st.markdown("---")
        st.markdown("### Interactive 3D MRI Volume View")
